...
double CAssembledRigidModel::refinePosition(const double maxPhiNorm,	const size_t nItersMax)
{
...
/** Solves the "finite displacement" problem: iterates refining the position until the constraints are minimized, keeping q[z_indices] fixed. */
double CAssembledRigidModel::finiteDisplacement(
	const std::vector<size_t> &z_indices, 
	const double maxPhiNorm, 
	const size_t nItersMax,
	bool also_correct_velocities,
	std::vector<size_t> * out_idxs_d)
{
	...
	// Return this precomputed list of dependent indices, to save time in the caller function.
	if (out_idxs_d)
		out_idxs_d->swap(idxs_d);
	...
	)
{
	...
	if (update_q)
	{
		...
		timelog.registerUserMeasure("computeDependentPosVelAcc.init_phi_norm", phi_norm);
		...
		timelog.registerUserMeasure("computeDependentPosVelAcc.num_iters", iter);
	}
	...
	// Update \ddot{q}
	// ------------------------------------------
	ASSERT_( (ptr_ddotz && out_results.ddotq) || (!ptr_ddotz && !out_results.ddotq) )
	if (ptr_ddotz)
	{
		...
		for (size_t i=0;i<nConstraints;i++)
		{
			...
			for (TCompressedRowSparseMatrix::row_t::const_iterator it=row_i.begin(); it!=row_i.end();++it)
			{
