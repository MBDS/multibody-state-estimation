...
// Ctor
CDynamicSimulatorIndepBase::CDynamicSimulatorIndepBase(CAssembledRigidModelPtr arm_ptr) :
...
void CDynamicSimulatorIndepBase::solve_ddotz(double t, VectorXd & ddot_z, bool can_choose_indep_coords)
{
	ASSERT_(m_init)
...
double CDynamicSimulatorIndepBase::run(const double t_ini, const double t_end)
{
	...
	for (t=t_ini;t<t_end;t+=t_step)
	{
		...
		case ODE_Euler:
			{
				this->internal_solve_ddotz(t, ddotz1, true /* can choose indep. coords */);
		...
		case ODE_RK4:
			{
				...
				// cur_time = t + t_step2;
				this->dq_plus_dz(v1,t_step2*ddotz1, m_arm->m_dotq); // \dot{q}= \dot{q}_0 + At/2 * \ddot{q}_1
				m_arm->m_q = q0 + t_step2*v1;
				this->correct_dependent_q_dq();

				v2 = m_arm->m_dotq;
				this->internal_solve_ddotz(t + t_step2, ddotz2, false /* don't change again the indep. coords */);
				...
				v3 = m_arm->m_dotq;
				this->internal_solve_ddotz(t + t_step2, ddotz3, false /* don't change again the indep. coords */);
				...
				v4 = m_arm->m_dotq;
				this->internal_solve_ddotz(t + t_step, ddotz4, false /* don't change again the indep. coords */);

				// Runge-Kutta 4th order formula:
				m_arm->m_q    = q0 + t_step6 * (v1+2*v2+2*v3+v4);
				this->dq_plus_dz(v1, t_step6 * (ddotz1+2*ddotz2+2*ddotz3+ddotz4), m_arm->m_dotq );
				this->correct_dependent_q_dq();
			}
			break;
...
void CDynamicSimulatorIndepBase::internal_solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
	ASSERTMSG_(lagrangre==NULL, "This solver uses independent coordinates, so it can't determine the lagrange multipliers as requested!")
