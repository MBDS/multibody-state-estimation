...
#include <sparsembs/CMultiBodyParticleFilter.h>
...
// Ctor:
CMultiBodyParticleFilter::CMultiBodyParticleFilter(const size_t M, const CModelDefinition &mbs)
{
	// 1) Proccess model:
...
// Dtor:
CMultiBodyParticleFilter::~CMultiBodyParticleFilter()
{
}
...
#pragma omp parallel for private(q0) private(k1) private(k2) private(k3) private(k4) private(v1) private(v2) private(v3) private(v4) private(ddotq1) private(ddotq2) private(ddotq3) private(ddotq4) private(q_incr) private(dotq_incr) private(dotq_noise)
#endif
		for (i=0;i<nParts;i++)
		{
			...
			// --------------------------------
			{
				q0 = part->num_model.m_q; // Make backup copy of state (velocities will be in "v1")

				// k1 = f(t,y);
				// cur_time = t;
				v1 = part->num_model.m_dotq;
				//No change needed: part->num_model.m_q = q0;
				part->dyn_simul->solve_ddotz(t, ddotz1, true /* can choose indep. coords */);

				// k2 = f(t+At/2,y+At/2*k1)
				// cur_time = t + t_step2;
				part->dyn_simul->dq_plus_dz(v1,t_step2*ddotz1, part->num_model.m_dotq); // \dot{q}= \dot{q}_0 + At/2 * \ddot{q}_1
				part->num_model.m_q = q0 + t_step2*v1;
				part->dyn_simul->correct_dependent_q_dq();

				v2 = part->num_model.m_dotq;
				part->dyn_simul->solve_ddotz(t+t_step2, ddotz2, false /* don't change again the indep. coords */);

				// k3 = f(t+At/2,y+At/2*k2)
				// cur_time = t + t_step2;
				//part->num_model.m_dotq = v1 + t_step2*ddotq2;  // \dot{q}= \dot{q}_0 + At/2 * \ddot{q}_2
				part->dyn_simul->dq_plus_dz(v1,t_step2*ddotz2, part->num_model.m_dotq);

				part->num_model.m_q = q0 + t_step2*v2;
				part->dyn_simul->correct_dependent_q_dq();

				v3 = part->num_model.m_dotq;
				part->dyn_simul->solve_ddotz(t+t_step2,ddotz3, false /* don't change again the indep. coords */);

				// k4 = f(t+At  ,y+At*k3)
				// cur_time = t + t_step;
				//part->num_model.m_dotq = v1 + t_step*ddotq3;
				part->dyn_simul->dq_plus_dz(v1, t_step*ddotz3, part->num_model.m_dotq);
				part->num_model.m_q = q0 + t_step*v3;
				part->dyn_simul->correct_dependent_q_dq();

				v4 = part->num_model.m_dotq;
				part->dyn_simul->solve_ddotz(t+t_step, ddotz4, false /* don't change again the indep. coords */);

				// Runge-Kutta 4th order formula:
				q_incr    = t_step6 * (v1+2*v2+2*v3+v4);
				dotz_incr = t_step6 * (ddotz1+2*ddotz2+2*ddotz3+ddotz4);
			}

			// generate noise:
			if (dotz_incr.size()!=dotz_noise.size()) dotz_noise.resize(dotz_incr.size());
			random_generator.drawGaussian1DMatrix(dotz_noise, 0, model_options.acc_xy_noise_std*t_step);

			// Add (noisy) increment:
			part->num_model.m_q    = q0 + q_incr;
			part->dyn_simul->dq_plus_dz(v1, dotz_incr + dotz_noise, part->num_model.m_dotq);
			part->dyn_simul->correct_dependent_q_dq();
...
//	sensors_loglik.reserve( nParts );
	...
	for (CParticleList::iterator it=m_particles.begin();it!=m_particles.end();++it)
	{
		...
		{
			const double log_lik = sensor_descriptions[k]->evaluate_log_likelihood( sensor_readings[k], part->num_model);
			cum_log_lik += log_lik;
		}
...
//	double sensor_avrg_lik = mrpt::math::chi2 CDF(nSensors,-mrpt::math::averageLogLikelihood(sensors_logw,sensors_loglik) );
...
void CMultiBodyParticleFilter::getAs3DRepresentation( mrpt::opengl::CSetOfObjectsPtr & 	outObj,const CBody::TRenderParams & rp ) const
{
	...
	for (CParticleList::const_iterator it=m_particles.begin();it!=m_particles.end();++it)
	{
		...
		mrpt::opengl::CSetOfObjectsPtr gl_part = mrpt::opengl::CSetOfObjects::Create();
		part->num_model.getAs3DRepresentation(gl_part,rp);
		outObj->insert(gl_part);
	}
}

void CMultiBodyParticleFilter::update3DRepresentation(const CBody::TRenderParams & rp_) const
{
	CBody::TRenderParams rp = rp_;
	...
	for (CParticleList::const_iterator it=m_particles.begin();it!=m_particles.end();++it)
	{
		...
		const uint8_t new_alpha = uint8_t( std::max(0.2, std::exp(it->log_w) ) * 255 );
