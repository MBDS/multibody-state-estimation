...
// ---------------------------------------------------------------------------------------------
CDynamicSimulator_Lagrange_CHOLMOD::CDynamicSimulator_Lagrange_CHOLMOD(const CAssembledRigidModelPtr arm_ptr)
	:
...
void CDynamicSimulator_Lagrange_CHOLMOD::internal_prepare()
{
	...
	// Since we'll factorize only definite matrices, it's OK to factor as LL' instead of LDL'
	...
	// ---------------------------------------
	cholmod_sparse *m_mass = cholmod_triplet_to_sparse(m_mass_tri,m_mass_tri->nnz, &m_cholmod_common);
	ASSERT_(m_mass!=NULL)

	m_cholmod_common.nmethods = 1;
	switch (this->ordering_M)
	{
	case orderNatural:      m_cholmod_common.method[0].ordering = CHOLMOD_NATURAL; break;
	case orderAMD:          m_cholmod_common.method[0].ordering = CHOLMOD_AMD; break;
	case orderMETIS:        m_cholmod_common.method[0].ordering = CHOLMOD_METIS; break;
	case orderNESDIS:        m_cholmod_common.method[0].ordering = CHOLMOD_NESDIS; break;
	case orderCOLAMD:       m_cholmod_common.method[0].ordering = CHOLMOD_COLAMD; break;
	default:
	...
	// -----------------------------------------------------------
	m_Phi_q_t_tri = cholmod_allocate_triplet(nDOFs,nConstraints, nDOFs*nConstraints, 0 /*unsymmetric*/, CHOLMOD_REAL, &m_cholmod_common);
	for (size_t i=0;i<nConstraints;i++)
	{
		// Constraint "i" goes to column "nDOFs+i" in the augmented matrix:
		const TCompressedRowSparseMatrix::row_t row_i = m_arm->m_Phi_q.matrix[i];
		for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_i.begin();itCol!=row_i.end();++itCol)
		{
			// We have precomputed the order in which we find the numeric values, just insert at their correct place:
			const size_t idx = m_Phi_q_t_tri->nnz;
			static_cast<int*>(m_Phi_q_t_tri->i)[idx] = itCol->first;
			static_cast<int*>(m_Phi_q_t_tri->j)[idx] = i;
			m_ptrs_Phi_q_t_tri.push_back( static_cast<double*>(m_Phi_q_t_tri->x)+idx );
			m_Phi_q_t_tri->nnz++;
		}
	}
	...
	m_Q = cholmod_allocate_dense(nDOFs,1,nDOFs, CHOLMOD_REAL, &m_cholmod_common);
	ASSERT_(m_Q!=NULL)
	m_c = cholmod_allocate_dense(nConstraints,1,nConstraints, CHOLMOD_REAL, &m_cholmod_common);
	ASSERT_(m_c!=NULL)


	// Build the structure of E once so we can build its symbolic decomposition just once now:
	// -------------------------------------------------------------------------------------------
	// Compress sparse matrix Phi_q_t:
	cholmod_sparse *Phi_q_t = cholmod_triplet_to_sparse(m_Phi_q_t_tri,m_Phi_q_t_tri->nnz, &m_cholmod_common);
	...
	//   Lm  *  E^t  = Phi_q^t
	//
	cholmod_sparse * E_t = cholmod_spsolve(CHOLMOD_L /*Lx=b*/, m_Lm, Phi_q_t, &m_cholmod_common );
	ASSERTDEB_(E_t!=NULL)
	cholmod_sparse * E = cholmod_transpose(E_t, 2 /* A' complex conjugate transpose */, &m_cholmod_common );
	ASSERTDEB_(E!=NULL)
...
void CDynamicSimulator_Lagrange_CHOLMOD::internal_solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
	...
	//static int k=0;
	//if (!k++) sparsembs::save_matrix(E,"E.txt",&m_cholmod_common);
	...
	// Solve: Lm x2 = Q
	cholmod_dense * x2 = cholmod_solve(CHOLMOD_L /*Lx=b*/, m_Lm, m_Q, &m_cholmod_common );
	ASSERTDEB_(x2!=NULL)

	// Solve: l2 = Lt \ (E*x2-c)
	double one [2]={1,0}, m1[2]={-1,0} ; // Scalars: 1 and -1
	cholmod_sdmult (E_t, 1 /*transpose of Et*/, one, m1, x2, m_c, &m_cholmod_common) ; /* c = E*x2 - c */

	cholmod_dense * l2 = cholmod_solve(CHOLMOD_L /*Lx=b*/, m_Lt, m_c, &m_cholmod_common );

	// Solve: Lt^t * l = l2
	cholmod_dense * l = cholmod_solve(CHOLMOD_Lt /*Ltx=b*/, m_Lt, l2, &m_cholmod_common );

	// Solve: x = Lm^t \ (x2-E_t*l)
	cholmod_sdmult(E_t, 0 /*don't transpose*/, m1, one, l, x2, &m_cholmod_common) ; /* x2 = x2 + E_t*l */

	cholmod_dense * x = cholmod_solve(CHOLMOD_Lt /*Ltx=b*/, m_Lm, x2, &m_cholmod_common );
	...
	if (lagrangre)
	{
		...
		memcpy(&(*lagrangre)[0], static_cast<double*>(x->x)+nDOFs, sizeof(double)*nConstraints);
	}
