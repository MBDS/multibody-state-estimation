...
-------------------------------------------------------------------*/
cholmod_triplet *CAssembledRigidModel::buildMassMatrix_sparse_CHOLMOD(cholmod_common &c) const
{
	...
	cholmod_triplet *triplet_M = cholmod_allocate_triplet(nDOFs,nDOFs, estimated_nnz,stype, CHOLMOD_REAL, &c);
	ASSERT_(triplet_M)
	...
	for (size_t i=0;i<parent_bodies.size();i++)
	{
		...
		const size_t idx_x0 = this->m_points2DOFs[b.points[0]].dof_x;  // Will be INVALID_DOF if it's a fixed point
		...
		if (!p0_fixed && !p1_fixed) 
			insert_submatrix_in_triplet(triplet_M,std::min(idx_x0,idx_x1), std::max(idx_x0,idx_x1), M01);
	...
	//cholmod_sparse *M = cholmod_triplet_to_sparse(triplet_M, triplet_M->nnz, &c);
	//timelog.leave("buildMassMatrix_sparse_CHOLMOD.ccs");
	//ASSERT_(M)
...
void CAssembledRigidModel::buildMassMatrix_dense( Eigen::MatrixXd &M ) const
{
	...
	for (size_t i=0;i<parent_bodies.size();i++)
	{
		...
		if (!p0_fixed) 
			M.block<2,2>(idx_x0,idx_x0) += M00;
		if (!p1_fixed) 
			M.block<2,2>(idx_x1,idx_x1) += M11;
		if (!p0_fixed && !p1_fixed) 
		{
			M.block<2,2>(std::min(idx_x0,idx_x1), std::max(idx_x0,idx_x1) ) = M01;
			M.block<2,2>(std::max(idx_x0,idx_x1), std::min(idx_x0,idx_x1) ) = M01.transpose();
		}
	} // end for each body
...
-------------------------------------------------------------------*/
void CAssembledRigidModel::buildMassMatrix_sparse( std::vector<Eigen::Triplet<double> > & tri ) const
{
	...
	for (size_t i=0;i<parent_bodies.size();i++)
	{
		...
		if (!p0_fixed)
			insert_submatrix(tri,idx_x0,idx_x0, M00);
		if (!p1_fixed) 
			insert_submatrix(tri,idx_x1,idx_x1, M11);
		if (!p0_fixed && !p1_fixed) 
		{
			insert_submatrix(tri,std::min(idx_x0,idx_x1), std::max(idx_x0,idx_x1), M01);
			insert_submatrix(tri,std::max(idx_x0,idx_x1), std::min(idx_x0,idx_x1), M01.transpose());
		}
	} // end for each body
