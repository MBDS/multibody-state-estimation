...
void CAssembledRigidModel::builGeneralizedForces( double *q ) const
{
	...
	for (size_t i=0;i<parent_bodies.size();i++)
	{
		...
		if (0)
		{
			...
			const TPoint2D p0(
				(p0_dofs.dof_x!=INVALID_DOF) ? m_q[p0_dofs.dof_x] : p0_info.coords.x,
				(p0_dofs.dof_y!=INVALID_DOF) ? m_q[p0_dofs.dof_y] : p0_info.coords.y );
			const TPoint2D p1(
				(p1_dofs.dof_x!=INVALID_DOF) ? m_q[p1_dofs.dof_x] : p1_info.coords.x,
				(p1_dofs.dof_y!=INVALID_DOF) ? m_q[p1_dofs.dof_y] : p1_info.coords.y );
		
			//const TPoint2D p1...
		}
		else
		{
			// Gravity force is always applied at the cog, whose coordinates are ALREADY stored as LOCAL COORDINATES:
		...
		const size_t idx_x0 = p0_dofs.dof_x;  // Will be INVALID_DOF if it's a fixed point
		const size_t idx_x1 = p1_dofs.dof_x;

		if (!p0_fixed)
			Q.segment<2>(idx_x0) += Qi.head<2>();
		if (!p1_fixed) 
			Q.segment<2>(idx_x1) += Qi.tail<2>();
	}
