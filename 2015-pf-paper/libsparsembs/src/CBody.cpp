...
*   M = [ ------+------ ]
*       [ M01^t |  M11  ]
*  In fact it's enough to fill the upper-triangular parts of M00 and M11.
*  M01 is not symmetric so it must be filled-in entirely.
*/
void CBody::evaluateMassMatrix( Matrix2d & M00, Matrix2d & M11, Matrix2d & M01 ) const
{
...
{
	// From ch3, eq(17) of "ANALISIS Y SINTESIS DE MECANISMOS POR ORDENADOR", Javier Cuadrado
	...
	// M00:
	m_M00(0,0) = m_mass-2*MXg_L + I0_L2;   m_M00(0,1) = 0;
	m_M00(1,0) = 0;                      m_M00(1,1) = m_M00(0,0);

	// M11:
	m_M11(0,0) = I0_L2;  m_M11(0,1) = 0;
	m_M11(1,0) = 0;      m_M11(1,1) = m_M11(0,0);

	// M01:
	m_M01(0,0) = MXg_L - I0_L2;  m_M01(0,1) = -MYg_L;
	m_M01(1,0) = MYg_L;          m_M01(1,1) = m_M01(0,0);
...
mrpt::opengl::CRenderizablePtr CBody::get3DRepresentation() const
{
	...
	case reCylinder:
		{
			...
			obj->setPose( mrpt::poses::CPose3D(0,0, render_params.z_layer, DEG2RAD(0),DEG2RAD(90),DEG2RAD(0) ) );
	...
	case reLine:
		{
			mrpt::opengl::CSimpleLinePtr obj = mrpt::opengl::CSimpleLine::Create();

			obj->setLineWidth(render_params.line_width);
			obj->enableAntiAliasing(true);
			obj->setColor_u8( mrpt::utils::TColor(0xFF,0xFF,0xFF, render_params.line_alpha) );

			obj->setLineCoords(0,0,render_params.z_layer, this->m_length,0,render_params.z_layer);
