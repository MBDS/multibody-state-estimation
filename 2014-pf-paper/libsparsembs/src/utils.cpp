...
bool sparsembs::save_matrix(cholmod_sparse *tri, const char *filename, cholmod_common *c)
{
...
bool sparsembs::save_matrix_dense(cholmod_sparse *tri, const char *filename, cholmod_common *c)
{
	FILE *f =mrpt::system::os::fopen(filename,"wt");
	if (!f) return false;
