...
//  Solver: Dense LU
// ---------------------------------------------------------------------------------------------
CDynamicSimulator_Indep_dense::CDynamicSimulator_Indep_dense(const CAssembledRigidModelPtr arm_ptr)
...
{
	m_arm->finiteDisplacement( m_indep_idxs, 1e-9, 20, true /* also solve dot{q} */);
}
...
{
	// In this model, independent accelerations are some selected indices out from
	// the vector of all dependent coordinates, so just add them:
	out_dq = dq;
	for (size_t i=0;i<m_indep_idxs.size();i++)
		out_dq[ m_indep_idxs[i] ] += dz[i];
}

// method: R matrix projection (as in section 5.2.3 of "J. García De Jalon & Bayo" book.
void CDynamicSimulator_Indep_dense::internal_solve_ddotz(double t, VectorXd & ddot_z, bool can_choose_indep_coords)
{
	...
	if (can_choose_indep_coords)
	{
		...
		//  by sorting the pivots of the LU decomposition (available via the permutation matrices):
		m_indep_idxs.resize(nDOFs);
		for (size_t i=0;i<nDOFs;i++)
			m_indep_idxs[i] = lu_Phiq.permutationQ().indices()[nDepCoords-nDOFs+i];
	...
	// This is a task of the integrator in double CDynamicSimulatorIndepBase::run(),
	// who must assure this.
