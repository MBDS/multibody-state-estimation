...
// ---------------------------------------------------------------------------------------------
//  Solver: Dense solver with the index-3 Augmented Lagrange formulation (ALF) with projection of velocities and acelerations
// ---------------------------------------------------------------------------------------------
CDynamicSimulator_ALi3_Dense::CDynamicSimulator_ALi3_Dense(const CAssembledRigidModelPtr arm_ptr)
	:
...
/** Implement a especific combination of dynamic formulation + integrator.
	...
	*/
bool CDynamicSimulator_ALi3_Dense::internal_integrate(double t, double dt, const ODE_integrator_t integr)
{
	...
	const Eigen::VectorXd qpp_g = -(4./dt2*m_arm->m_q + 4./dt*m_arm->m_dotq + m_arm->m_ddotq);
	...
	while (err>tol_dyn && iter < iter_max)
	{
		...
		Eigen::VectorXd RHS = 0.25*dt2* ( m_M*m_arm->m_ddotq+m_Phi_q.transpose()*params_penalty.alpha* m_arm->m_Phi+m_Phi_q.transpose()*m_Lambda - Q );
		//[K,C]=evalKC(k_m, c_m);

		//f_q = M + 0.5*dt*C+0.25*dt^2*(jac'*alpha*jac+K);
		m_A = m_M + 0.25*dt2*params_penalty.alpha * m_Phi_q.transpose()*m_Phi_q;
		m_A_lu.compute(m_A);
	...
	//qpp_out = f_q\((M + 0.5*dt*C + 0.25*dt^2*K)*qpp - 0.25*dt^2*jac'*alpha*phiqpqp_0);
	m_arm->m_ddotq = m_A_lu.solve( m_M * m_arm->m_ddotq - 0.25*dt2*params_penalty.alpha* m_Phi_q.transpose() *  m_dotPhi_q * m_arm->m_dotq );
...
void CDynamicSimulator_ALi3_Dense::internal_solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
	const size_t nDepCoords = m_arm->m_q.size();
	...
	//                =A
	//
	// RHS = Q(q,dq) - alpha * Phi_q^t* [ \dot{Phi}_q * \dot{q} + 2 * xi * omega * \dot{Phi} + omega^2 * Phi ] - Phi_q^t * \lambda
	//
	...
	Eigen::MatrixXd RHS = Q - params_penalty.alpha * m_Phi_q.transpose()*
		(
