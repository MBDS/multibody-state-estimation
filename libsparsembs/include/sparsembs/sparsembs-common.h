...
/** \file  This file includes common stuff required by all files in the MBS C++ lib.
 */
...
namespace sparsembs
{
	...
	struct TDOF
	{
		...
		TDOF(size_t _point_index, size_t _point_dof) : point_index(_point_index), point_dof(_point_dof) {}
	};
	...
	struct TPoint2DOF
	{
		size_t dof_x; //!< The 0-based index in vector q of the "x" coordinate of this point (-1 if it's a fixed point)
		size_t dof_y; //!< The 0-based index in vector q of the "y" coordinate of this point (-1 if it's a fixed point)
	...
	struct TCompressedRowSparseMatrix
	{
		...
		size_t  ncols; //!< The number of cols in a sparse matrix can be set freely by the user (we don't check for columns out of this limit anyway)
		...
		void getAsDense( MATRIX &M ) const
		{
			...
			{
				for (row_t::const_iterator it=matrix[i].begin();it!=matrix[i].end();++it)
				{
