...
// ---------------------------------------------------------------------------------------------
CDynamicSimulator_Lagrange_UMFPACK::CDynamicSimulator_Lagrange_UMFPACK(const CAssembledRigidModelPtr arm_ptr)
	:
...
void CDynamicSimulator_Lagrange_UMFPACK::internal_prepare()
{
	...
	// -----------------------------------------------------------
	m_A_tri.reserve(m_A_tri.size() + 2*nConstraints*nDOFs); // *IMPORTANT* Reserve mem at once to avoid reallocations, since we store pointers to places...
	...
	for (size_t i=0;i<nConstraints;i++)
	{
		...
		for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_i.begin();itCol!=row_i.end();++itCol)
		{
			...
			m_A_tri.push_back( Eigen::Triplet<double>(itCol->first,nDOFs+i, 1.0) );
			m_A_tri.push_back( Eigen::Triplet<double>(nDOFs+i,itCol->first, 1.0) );

			m_A_tri_ptrs_Phi_q.push_back( const_cast<double*>( &m_A_tri[idx0].value() ) );
			m_A_tri_ptrs_Phi_q.push_back( const_cast<double*>( &m_A_tri[idx0+1].value() ) );
		}
	}
	...
	{
	case orderNatural:      m_umf_control[UMFPACK_ORDERING] = UMFPACK_ORDERING_NONE; break;
	case orderAMD:          m_umf_control[UMFPACK_ORDERING] = UMFPACK_ORDERING_AMD; break;
	case orderMETIS:        m_umf_control[UMFPACK_ORDERING] = UMFPACK_ORDERING_METIS; break;
	case orderCHOLMOD:      m_umf_control[UMFPACK_ORDERING] = UMFPACK_ORDERING_CHOLMOD; break;
	case orderTryKeepBest:  m_umf_control[UMFPACK_ORDERING] = UMFPACK_ORDERING_BEST; break;
	default:
		THROW_EXCEPTION("Unknown or unsupported 'ordering' value.")
	};

	int errorCode = umfpack_symbolic(
		m_A.rows(), m_A.cols(), m_A.outerIndexPtr(), m_A.innerIndexPtr(), m_A.valuePtr(),
        &m_symbolic, m_umf_control, m_umf_info);

	if (errorCode<0)
		THROW_EXCEPTION("Error: UMFPACK couldn't factorize the augmented matrix.")
...
void CDynamicSimulator_Lagrange_UMFPACK::internal_solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
    ...
    int errorCode = umfpack_di_numeric(
		m_A.outerIndexPtr(), m_A.innerIndexPtr(), m_A.valuePtr(),
		m_symbolic, &m_numeric,m_umf_control, m_umf_info);

	if (errorCode<0)
		THROW_EXCEPTION("Error: UMFPACK couldn't numeric-factorize the augmented matrix.")
    ...
    errorCode = umfpack_di_solve(
		...
		&solution[0], &RHS[0],
		m_numeric,
		m_umf_control, m_umf_info);

    if (errorCode!=0)
	{
		mrpt::math::saveEigenSparseTripletsToFile("DUMP_UMFPACK_ERROR_A.txt",m_A_tri);
