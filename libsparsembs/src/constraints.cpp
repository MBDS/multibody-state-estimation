...
/* -------------------------------------------------------------------
   ...
   -------------------------------------------------------------------*/
void CConstraintConstantDistance::buildSparseStructures(CAssembledRigidModel &arm) const
{
	m_points[0] = &arm.m_parent.getPointInfo(this->point_index0);
	m_points[1] = &arm.m_parent.getPointInfo(this->point_index1);

	ASSERTMSG_(! (m_points[0]->fixed && m_points[1]->fixed), "Useless constraint added between two fixed points!")
	...
	if (!m_points[0]->fixed)  // Only for the variables, not fixed points:
	{
		dot_dPhi_dx0 = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOFs[0].dof_x ];
		dot_dPhi_dy0 = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOFs[0].dof_y ];
	}
	if (!m_points[1]->fixed)
	{
		dot_dPhi_dx1 = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOFs[1].dof_x ];
		dot_dPhi_dy1 = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOFs[1].dof_y ];
	}
...
void CConstraintConstantDistance::update(CAssembledRigidModel &arm) const
{
	...
	const double & p0x = (m_pointDOFs[0].dof_x!=INVALID_DOF) ? arm.m_q[m_pointDOFs[0].dof_x] : m_points[0]->coords.x ;
	const double & p0y = (m_pointDOFs[0].dof_y!=INVALID_DOF) ? arm.m_q[m_pointDOFs[0].dof_y] : m_points[0]->coords.y ;

	const double & p1x = (m_pointDOFs[1].dof_x!=INVALID_DOF) ? arm.m_q[m_pointDOFs[1].dof_x] : m_points[1]->coords.x ;
	const double & p1y = (m_pointDOFs[1].dof_y!=INVALID_DOF) ? arm.m_q[m_pointDOFs[1].dof_y] : m_points[1]->coords.y ;

	// Get references to point velocities (fixed=>Zero, variables=>their actual members in m_dotq):
	const double dummy_zero = 0;

	const double & p0dotx = (m_pointDOFs[0].dof_x!=INVALID_DOF) ? arm.m_dotq[m_pointDOFs[0].dof_x] : dummy_zero;
	const double & p0doty = (m_pointDOFs[0].dof_y!=INVALID_DOF) ? arm.m_dotq[m_pointDOFs[0].dof_y] : dummy_zero;

	const double & p1dotx = (m_pointDOFs[1].dof_x!=INVALID_DOF) ? arm.m_dotq[m_pointDOFs[1].dof_x] : dummy_zero;
	const double & p1doty = (m_pointDOFs[1].dof_y!=INVALID_DOF) ? arm.m_dotq[m_pointDOFs[1].dof_y] : dummy_zero;
...
/* -------------------------------------------------------------------
   ...
   -------------------------------------------------------------------*/
void CConstraintFixedSlider::buildSparseStructures(CAssembledRigidModel &arm) const
{
	m_Delta = line_pt[1] - line_pt[0];
	ASSERTMSG_(m_Delta.norm()>0, "Error: the two constraint points must define a line but they coincide")
...
/** Creates a 3D representation of the constraint, if applicable (e.g. the line of a fixed slider)
  * \return false if the constraint has no 3D representation
  */
bool CConstraintFixedSlider::get3DRepresentation( mrpt::opengl::CRenderizablePtr & inout_obj ) const
{
	inout_obj=mrpt::opengl::CSimpleLine::Create(
		line_pt[0].x,line_pt[0].y,0,
		line_pt[1].x,line_pt[1].y,0,
		1 /*line width*/ );
...
/* -------------------------------------------------------------------
   ...
   -------------------------------------------------------------------*/
void CConstraintMobileSlider::buildSparseStructures(CAssembledRigidModel &arm) const
{
	...
	ASSERTMSG_(!m_points[0]->fixed, "Useless constraint added to a fixed point!")
	...
	for (int i=0;i<3;i++)
	{
		if (!m_points[i]->fixed)  // Only for the variables, not fixed points:
		{
			dot_dPhi_dx[i] = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOF[i].dof_x ];
			dot_dPhi_dy[i] = & arm.m_dotPhi_q.matrix[m_idx_constr][ m_pointDOF[i].dof_y ];
		}
	}
}
...
{
	// Get references to the point coordinates (either fixed or variables in q):
	const double & px = (m_pointDOF[0].dof_x!=INVALID_DOF) ? arm.m_q[m_pointDOF[0].dof_x] : m_points[0]->coords.x ;
	const double & py = (m_pointDOF[0].dof_y!=INVALID_DOF) ? arm.m_q[m_pointDOF[0].dof_y] : m_points[0]->coords.y ;

	const double & pxr0 = (m_pointDOF[1].dof_x!=INVALID_DOF) ? arm.m_q[m_pointDOF[1].dof_x] : m_points[1]->coords.x ;
	const double & pyr0 = (m_pointDOF[1].dof_y!=INVALID_DOF) ? arm.m_q[m_pointDOF[1].dof_y] : m_points[1]->coords.y ;

	const double & pxr1 = (m_pointDOF[2].dof_x!=INVALID_DOF) ? arm.m_q[m_pointDOF[2].dof_x] : m_points[2]->coords.x ;
	const double & pyr1 = (m_pointDOF[2].dof_y!=INVALID_DOF) ? arm.m_q[m_pointDOF[2].dof_y] : m_points[2]->coords.y ;


	// Get references to point velocities (fixed=>Zero, variables=>their actual members in m_dotq):
	const double dummy_zero = 0;

	const double & vx = (m_pointDOF[0].dof_x!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[0].dof_x] : dummy_zero;
	const double & vy = (m_pointDOF[0].dof_y!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[0].dof_y] : dummy_zero;

	const double & vxr0 = (m_pointDOF[1].dof_x!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[1].dof_x] : dummy_zero;
	const double & vyr0 = (m_pointDOF[1].dof_y!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[1].dof_y] : dummy_zero;

	const double & vxr1 = (m_pointDOF[2].dof_x!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[2].dof_x] : dummy_zero;
	const double & vyr1 = (m_pointDOF[2].dof_y!=INVALID_DOF) ? arm.m_dotq[m_pointDOF[2].dof_y] : dummy_zero;

	// Update Phi[i]
	// ----------------------------------
	arm.m_Phi[ m_idx_constr ] = (pxr1-pxr0)*(py-pyr0) - (pyr1-pyr0)*(px-pxr0);

	// Update dotPhi[i]
	// ----------------------------------
	arm.m_dotPhi[m_idx_constr] = (vxr1-vxr0)*(py-pyr0)+(pxr1-pxr0)*(vy-vyr0) - (vyr1-vyr0)*(px-pxr0)-(pyr1-pyr0)*(vx-vxr0);
