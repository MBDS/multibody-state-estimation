...
{
	for (int i=0;i<3;i++)
		m_gravity[i] = DEFAULT_GRAVITY[i];

	const size_t nDOFs = armi.DOFs.size();
	ASSERTMSG_(nDOFs>0, "Trying to assemble model with 0 DOFs!?")

	m_q.resize(nDOFs); m_q.setConstant(0);
	m_dotq.resize(nDOFs); m_dotq.setConstant(0);
	m_ddotq.resize(nDOFs); m_ddotq.setConstant(0);
	m_DOFs = armi.DOFs;
	...
	// ---------------------------------------------------------------------------------------------------
	const std::vector<CConstraintBasePtr> & parent_constraints = m_parent.getConstraints();
...
void CAssembledRigidModel::getGravityVector(double &gx,double &gy,double &gz) const
{
	gx = m_gravity[0];
	gy = m_gravity[1];
	gz = m_gravity[2];
...
void CAssembledRigidModel::setGravityVector(const double gx,const double gy,const double gz)
{
	m_gravity[0] = gx;
	m_gravity[1] = gy;
	m_gravity[2] = gz;
...
void CAssembledRigidModel::getAs3DRepresentation( mrpt::opengl::CSetOfObjectsPtr & 	outObj, const CBody::TRenderParams & rp ) const
{
	using namespace mrpt::utils;
	...
	// Render constraints:
	const std::vector<CConstraintBasePtr> & parent_constr = m_parent.getConstraints();
	...
	for (size_t i=0;i<nBodies;i++)
	{
		...
		// And save reference for quickly update the 3D pose in the future during animations:
...
/** Animates a 3D representation of the MBS, previously built in getAs3DRepresentation()
	...
	*/
void CAssembledRigidModel::update3DRepresentation(const CBody::TRenderParams & rp) const
{
	const std::vector<CBody> &parent_bodies = m_parent.getBodies();
	...
	for (size_t i=0;i<nBodies;i++)
	{
		...
		const double & p0x = (dof0.dof_x!=INVALID_DOF) ? m_q[dof0.dof_x] : pnt0.coords.x;
		const double & p0y = (dof0.dof_y!=INVALID_DOF) ? m_q[dof0.dof_y] : pnt0.coords.y;

		const double & p1x = (dof1.dof_x!=INVALID_DOF) ? m_q[dof1.dof_x] : pnt1.coords.x;
		const double & p1y = (dof1.dof_y!=INVALID_DOF) ? m_q[dof1.dof_y] : pnt1.coords.y;
		...
		obj->setPose( mrpt::poses::CPose3D(
			p0x,p0y,0,
			theta, DEG2RAD(0),DEG2RAD(0) ) );

		// Update transparency:
		if (rp.render_style == CBody::reLine)
		{
			mrpt::opengl::CSimpleLinePtr gl_line = mrpt::opengl::CSetOfObjectsPtr(obj)->getByClass<mrpt::opengl::CSimpleLine>();
			if (gl_line)
				gl_line->setColorA_u8(rp.line_alpha);
		}
	}
}
...
#ifdef _DEBUG
	ASSERT_(ptr_q0 == &m_q[0]) // make sure the vectors didn't suffer mem reallocation, since we save pointers to these!
	ASSERT_(ptr_dotq0 == &m_dotq[0]) // make sure the vectors didn't suffer mem reallocation, since we save pointers to these!
#endif
}

/** Copies the opengl object from another instance */
void CAssembledRigidModel::copyOpenGLRepresentationFrom(const CAssembledRigidModel &o)
{
...
void CAssembledRigidModel::getPhi_q_dense(Eigen::MatrixXd &Phi_q) const
{
	...
	for (size_t i=0;i<nConstraints;i++)
	{
		...
		for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_i.begin();itCol!=row_i.end();++itCol)
...
void CAssembledRigidModel::getPointCurrentCoords(const size_t pt_idx, mrpt::math::TPoint2D &pt) const
{
	const TMBSPoint  & pt_info = m_parent.getPointInfo(pt_idx);
	const TPoint2DOF & pt_dofs = m_points2DOFs[pt_idx];

	pt.x = (pt_dofs.dof_x!=INVALID_DOF) ? m_q[pt_dofs.dof_x] : pt_info.coords.x;
	pt.y = (pt_dofs.dof_y!=INVALID_DOF) ? m_q[pt_dofs.dof_y] : pt_info.coords.y;
}

/** Retrieves the current velocity of a point, which may include either fixed or variable components */
void CAssembledRigidModel::getPointCurrentVelocity(const size_t pt_idx, mrpt::math::TPoint2D &vel) const
{
	const TPoint2DOF & pt_dofs = m_points2DOFs[pt_idx];
...
void CAssembledRigidModel::getPointOnBodyCurrentCoords(const size_t body_index, const mrpt::math::TPoint2D &relative_pt, mrpt::math::TPoint2D &out_pt) const
{
...
mrpt::opengl::CSetOfObjectsPtr CAssembledRigidModel::internal_render_ground_point(const TMBSPoint &pt, const CBody::TRenderParams & rp) const
{
	...
	mrpt::opengl::CBoxPtr gl_box = mrpt::opengl::CBox::Create( mrpt::math::TPoint3D(-0.5*support_LXZ,-support_LY,-0.5*support_LXZ),mrpt::math::TPoint3D(0.5*support_LXZ,0,0.5*support_LXZ), false );
...
void CAssembledRigidModel::evaluateEnergy(CAssembledRigidModel::TEnergyValues &e) const
{
	...
	for (size_t i=0;i<bodies.size();i++)
	{
		...
		e.E_kin += 0.5 * (dq0.transpose() * M00 * dq0 + dq1.transpose() * M11 * dq1 ).coeff(0,0) + (dq0.transpose() * M01 * dq1).coeff(0,0);

		// Potential energy:
		mrpt::math::TPoint2D global_cog; // current COG position, in global coords:
		this->getPointOnBodyCurrentCoords(i, b.cog(), global_cog);

		e.E_pot -= b.mass()*(this->m_gravity[0] * global_cog.x + this->m_gravity[1] * global_cog.y); // + this->m_gravity[2] * global_cog.z
	}
