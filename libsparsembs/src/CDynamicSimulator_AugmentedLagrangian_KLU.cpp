...
// ---------------------------------------------------------------------------------------------
CDynamicSimulator_AugmentedLagrangian_KLU::CDynamicSimulator_AugmentedLagrangian_KLU(const CAssembledRigidModelPtr arm_ptr)
	:
	CDynamicSimulatorBasePenalty(arm_ptr),
	ordering(orderAMD),
	m_numeric(NULL), m_numeric_M(NULL),
	m_symbolic(NULL), m_symbolic_M(NULL)
{
...
void CDynamicSimulator_AugmentedLagrangian_KLU::internal_prepare()
{
	...
	//                =A
	//
	// RHS = Q - alpha * Phi_q^t* [ \dot{Phi}_q * \dot{q} + 2 * xi * omega * \dot{q} + omega^2 * Phi  ]
	//
	// Build mass matrix (constant), and set its triplet form as the beginning of the total "A" matrix:
	m_arm->buildMassMatrix_sparse(m_M_tri);
	m_A_tri = m_M_tri;

	//  Add entries in the triplet form for the sparse Phi_q Jacobian.
	// -----------------------------------------------------------
	m_A_tri.reserve(m_A_tri.size() + nDepCoords+ nDepCoords*nDepCoords); // *IMPORTANT* Reserve mem at once to avoid reallocations, since we store pointers to places...

	m_PhiqtPhi.clear();
	m_PhiqtPhi.reserve(nDepCoords*nDepCoords);

	// Note: All this could be done much more efficiently if Phi_q was stored
	// in compressed column form. But since this is only computed ONCE per simulation
	// it's probably worth leave it stay...
	for (size_t i=0;i<nDepCoords;i++)
	{
		for (size_t j=i;j<nDepCoords;j++)
		{
			// We have to evaluate the "dot product" of the columns i and j of Phi_q:
			TSparseDotProduct sdp;

			for (size_t row=0;row<nConstraints;row++)
			{
				const TCompressedRowSparseMatrix::row_t &row_r = m_arm->m_Phi_q.matrix[row];

				const double* Phi_r_i = NULL, *Phi_r_j = NULL;

				for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_r.begin();itCol!=row_r.end();++itCol)
				{
				...
				if ( Phi_r_i && Phi_r_j)
					sdp.lst_terms.push_back( std::pair<const double*,const double*>(Phi_r_i,Phi_r_j) );
			} // end for each "row"
			...
			if (!sdp.lst_terms.empty())
			{
				...
				if (i!=j)
				{
					...
					sdp.out_ptr2 = const_cast<double*>( &(m_A_tri.back().value()) );
				}
	...
	{
	case orderAMD:          m_common.ordering = 0; break;
	case orderCOLAMD:       m_common.ordering = 1; break;
	default:
		THROW_EXCEPTION("Unknown or unsupported 'ordering' value.")
	};

	m_symbolic = klu_analyze (m_A.rows(),m_A.outerIndexPtr(), m_A.innerIndexPtr(), &m_common);
	if (!m_symbolic)
		THROW_EXCEPTION("Error: KLU couldn't factorize the augmented matrix.")

	// Mass matrix: factorize numerically since it's constant:
	m_symbolic_M = klu_analyze (m_M.rows(),m_M.outerIndexPtr(), m_M.innerIndexPtr(), &m_common);
	if (!m_symbolic_M)
		THROW_EXCEPTION("Error: KLU couldn't factorize the mass matrix.")
	m_numeric_M = klu_factor (m_M.outerIndexPtr(), m_M.innerIndexPtr(), m_M.valuePtr(), m_symbolic_M, &m_common) ;

	timelog.leave("solver_prepare");
}


CDynamicSimulator_AugmentedLagrangian_KLU::~CDynamicSimulator_AugmentedLagrangian_KLU()
{
...
void CDynamicSimulator_AugmentedLagrangian_KLU::internal_solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
	const size_t nDepCoords = m_arm->m_q.size();
	const size_t nConstraints = m_arm->m_Phi.size();
	...
	klu_solve(m_symbolic_M, m_numeric_M, m_M.cols(), 1, &ddotq_prev[0], &m_common) ;
	...
	m_numeric = klu_factor (m_A.outerIndexPtr(), m_A.innerIndexPtr(), m_A.valuePtr(), m_symbolic, &m_common) ;

	if (!m_numeric)
		THROW_EXCEPTION("Error: KLU couldn't numeric-factorize the augmented matrix.")
	timelog.leave("solver_ddotq.numeric_factor");

	// Build the RHS vector:
	// RHS = M*\ddot{q}_i -  Phi_q^t* alpha * [ \dot{Phi}_q * \dot{q} + 2 * xi * omega * \dot{q} + omega^2 * Phi  ]
	//                               \ ------------------------------------v --------------------------------------/
	//                                                                    = b
	timelog.enter("solver_ddotq.build_rhs");

	// Evaluate "b":
	// b = alpha * [ \dot{Phi}_q * \dot{q} + 2 * xi * omega * \dot{Phi} + omega^2 * Phi  ]
	Eigen::VectorXd b(nConstraints);

	// \dot{Phi}_q * \dot{q}
	for (size_t r=0;r<nConstraints;r++)
	{
		const TCompressedRowSparseMatrix::row_t &row_r = m_arm->m_dotPhi_q.matrix[r];
		double res=0;
		for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_r.begin();itCol!=row_r.end();++itCol)
	...
	for (size_t r=0;r<nConstraints;r++)
	{
		const TCompressedRowSparseMatrix::row_t &row_r = m_arm->m_Phi_q.matrix[r];
		for (TCompressedRowSparseMatrix::row_t::const_iterator itCol=row_r.begin();itCol!=row_r.end();++itCol)
		{
			const size_t col=itCol->first;
	...
	do
	{
		...
		klu_solve(m_symbolic, m_numeric, m_A.cols(), 1, &ddotq_next[0], &m_common) ;
