...
/** A class factory, creates a dynamic simulator from a string with the class name: "CDynamicSimulator_Lagrange_LU_dense", "CDynamicSimulator_Lagrange_UMFPACK", ...
	*/
CDynamicSimulatorBasePtr CDynamicSimulatorBase::Create(const std::string &name,const CAssembledRigidModelPtr arm_ptr)
{
	if (name=="CDynamicSimulator_Lagrange_LU_dense") return CDynamicSimulatorBasePtr(new CDynamicSimulator_Lagrange_LU_dense(arm_ptr));
	else if (name=="CDynamicSimulator_Lagrange_CHOLMOD") return CDynamicSimulatorBasePtr(new CDynamicSimulator_Lagrange_CHOLMOD(arm_ptr));
	else if (name=="CDynamicSimulator_Lagrange_UMFPACK") return CDynamicSimulatorBasePtr(new CDynamicSimulator_Lagrange_UMFPACK(arm_ptr));
	else if (name=="CDynamicSimulator_Lagrange_KLU") return CDynamicSimulatorBasePtr(new CDynamicSimulator_Lagrange_KLU(arm_ptr));
	else THROW_EXCEPTION("Unknown dynamic simulator class name: "+ name);
}
...
void CDynamicSimulatorBase::solve_ddotq(double t, VectorXd & ddot_q, VectorXd * lagrangre )
{
	ASSERT_(m_init)
...
double CDynamicSimulatorBase::run(const double t_ini, const double t_end)
{
	...
	for (t=t_ini;t<t_end;t+=t_step)
	{
		// Log sensor points:
		// ------------------------------
		for (std::list<TSensorData>::iterator it=m_sensors.begin();it!=m_sensors.end();++it)
		{
			TSensorData &sd = *it;
			sd.log.push_back(
				timestamped_point_t(
		...
		const bool custom_integrator = this->internal_integrate(t,t_step,params.ode_solver);
		...
		if (!custom_integrator)
		{
			...
			case ODE_RK4:
				{
					q0 = m_arm->m_q; // Make backup copy of state (velocities will be in "v1")
					...
					// cur_time = t + t_step2;
					m_arm->m_dotq = v1 + t_step2*ddotq1;  // \dot{q}= \dot{q}_0 + At/2 * \ddot{q}_1
					...
					// cur_time = t + t_step2;
					m_arm->m_dotq = v1 + t_step2*ddotq2;  // \dot{q}= \dot{q}_0 + At/2 * \ddot{q}_2
					...
					m_arm->m_dotq = v1 + t_step6 * (ddotq1+2*ddotq2+2*ddotq3+ddotq4);
				}
				break;
...
void CDynamicSimulatorBase::build_RHS(double *Q, double *c)
{
	...
	if (c)
	{
		...
		for (size_t i=0;i<nConstraints;i++)
		{
			...
			double ci=0;
			const TCompressedRowSparseMatrix::row_t row_i = m_arm->m_dotPhi_q.matrix[i];
		...
		for (size_t i=0;i<nConstraints;i++)
			c[i] -= 2*epsilon*omega* m_arm->m_dotPhi[i] + omega*omega* m_arm->m_Phi[i];
#endif
	}
...
void CDynamicSimulatorBase::addPointSensor(const size_t pnt_index)
{
	...
	sd.pos[0] = ((point_dof.dof_x!=INVALID_DOF) ? &m_arm->m_q[point_dof.dof_x] : &mbs_point->coords.x);
	sd.pos[1] = ((point_dof.dof_y!=INVALID_DOF) ? &m_arm->m_q[point_dof.dof_y] : &mbs_point->coords.y);

	// Get references to the point velocities:
	sd.vel[0] = ((point_dof.dof_x!=INVALID_DOF) ? &m_arm->m_dotq[point_dof.dof_x] : &dummy_zero);
	sd.vel[1] = ((point_dof.dof_y!=INVALID_DOF) ? &m_arm->m_dotq[point_dof.dof_y] : &dummy_zero);
...
/** Save all logged data to a text file, loadable from MATLAB with "load()".
  ...
  */
bool CDynamicSimulatorBase::saveSensorLogsToFile(const std::string &filename) const
{
	...
	size_t N=0;
	f << "% Time ";
	for (std::list<TSensorData>::const_iterator it=m_sensors.begin();it!=m_sensors.end();++it)
	{
	...
	for (size_t i=0;i<N;i++)
	{
		...
		{
			const TSensorData &sd = *it;
			s+= mrpt::format("%f\t %f\t %f\t %f\t ",
				sd.log[i].second.pos.x, sd.log[i].second.pos.y,
				sd.log[i].second.vel.x, sd.log[i].second.vel.y );
		}
		s+=std::string("\n");
		f << s;
	}
