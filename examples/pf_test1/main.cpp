...
// ===================================================================
const size_t  NUM_PARTS = getenv("PF_NUM_SAMPLES")!=NULL ? atoi(getenv("PF_NUM_SAMPLES")) : 500;

// PF's sensor model: 1 sigma of the assumed noise (in meters)
const double  PF_MOTION_MODEL_NOISE_XY =  getenv("PF_MOTION_MODEL_NOISE_XY")!=NULL ? atof(getenv("PF_MOTION_MODEL_NOISE_XY")) : 1;

// Gyro's assumed noise (1 sigma) (deg/s)
const double  PF_SENSOR_NOISE_STD = DEG2RAD( getenv("PF_SENSOR_NOISE_STD")!=NULL ? atof(getenv("PF_SENSOR_NOISE_STD")) : 10.0 ); // PF's sensor model: 1 sigma of the assumed noise (in meters)

// Real noise added to simulated sensor
const double  REAL_SENSOR_STD     = DEG2RAD(0.3);

// Time step lenght (in seconds):
const double  SIMUL_STEP = getenv("SIMUL_STEP")!=NULL ? atof(getenv("SIMUL_STEP")) : 5e-3;

const int     DRAW_DECIMATION = 1;
const int     DRAW_DELAY_MS   = getenv("DRAW_DELAY_MS")!=NULL ? atoi(getenv("DRAW_DELAY_MS")) : 1;

// Optionally: modify mass of body #1 by a factor different than 1.0 to force the PF to track an incorrectly modeled system:
const double  IMPERFECT_PF_MODEL_ERROR = getenv("IMPERFECT_PF_MODEL_ERROR")!=NULL ? atof(getenv("IMPERFECT_PF_MODEL_ERROR")) : 1;
...
void buildFollowerMBS(CModelDefinition &model )
{
	...
	model.addConstraint( CConstraintMobileSlider(
		1 /*pt index*/,
		2,3 /* two pts for defining the constraint */
		) );

	model.addConstraint( CConstraintFixedSlider(
		4 /*pt index*/,
		TPoint2D(-5,0), TPoint2D(10,0) /* The line on which to fix the point */
		) );

}


void pf_initialize_uniform_distribution(CMultiBodyParticleFilter &pf,CModelDefinition &model);
...
int main(int argc, char **argv)
{
	try
	{
		...
		// "Compile" the problem, and use as "Ground Truth" (GT) for the estimation problem
		// -------------------------------------------------------------------------------------
		...
		// Corrupt PF model:
		{
			...
			bodies[1].mass()   *= error;
			bodies[1].I0()     = (1./3.)*bodies[1].mass()*square(bodies[1].length());
		...
		// PF must be initialized with a uniform distribution (no "prior" at all)
		...
		mrpt::opengl::CSetOfObjectsPtr gl_MBS = mrpt::opengl::CSetOfObjects::Create();
		aMBS_GT->getAs3DRepresentation( gl_MBS, rp );

		mrpt::opengl::CSetOfObjectsPtr gl_MBPF = mrpt::opengl::CSetOfObjects::Create();
		...
		mrpt::opengl::COpenGLViewportPtr gl_estimate;
		{
			...
			gl_main->insert( mrpt::opengl::stock_objects::CornerXYZSimple(0.2,1));
			gl_main->insert(gl_MBS);

			win3D.addTextMessage(
				10,0.95, "Ground truth",
				TColorf(0,0,1), "mono",20,  mrpt::opengl::NICE, 1 /* txt ID */, 1.5 /*spacing*/, 0.1 /*kerning*/, true /*draw shadow */ );

			// Estimate:
			gl_estimate->setViewportPosition(0,0,1.0,0.5);
			gl_estimate->setBorderSize(1);
			gl_estimate->insert( mrpt::opengl::stock_objects::CornerXYZSimple(0.2,1) );
			gl_estimate->insert( gl_MBPF );

			win3D.addTextMessage(
				10,0.45, "Estimation",
				TColorf(0,0,1), "mono",20,  mrpt::opengl::NICE, 2 /* txt ID */, 1.5 /*spacing*/, 0.1 /*kerning*/, true /*draw shadow */ );
	...
	// Save initial state:
		...
		orientation_averager.resetDeterministic( CPose2D(), pf.m_particles.size());
#endif
		...
		// Gyroscope on body #2:
//		sensor_descriptions.push_back( CVirtualSensorPtr(new CVirtualSensor_Gyro(2 /* body index */)) );
//		sensor_descriptions.back()->sensor_noise_std = PF_SENSOR_NOISE_STD;

		// Gyroscope on body #1:
		sensor_descriptions.push_back( CVirtualSensorPtr(new CVirtualSensor_Gyro(1 /* body index */)) );
		sensor_descriptions.back()->sensor_noise_std = PF_SENSOR_NOISE_STD;
		...
		while (t_old_simul<FINAL_TIME)
		{
			...
			const double t_pf_start = t_old_simul;
			{
				...
				// Simulate: (Return the actual end-time of the simulation, >=t_new)
				const double t_final = dynSimul_GT.run(t_old_simul,t_new_simul);
				t_old_simul=t_final;
			}
			...
			for (size_t j=0;j<nSensors;j++)
				sensor_readings[j] = sensor_descriptions[j]->simulate_reading(*aMBS_GT) + mrpt::random::randomGenerator.drawGaussian1D(0,REAL_SENSOR_STD);

			// Run the PF models:
			// ==============================================
			CMultiBodyParticleFilter::TOutputInfo pf_out_info;
			pf.run_PF_step(t_pf_start, t_pf_end, SIMUL_STEP, sensor_descriptions, sensor_readings, pf_out_info);
			...
			// GT value:
			GT_ang0.push_back( atan2( aMBS_GT->m_q[aMBS_GT->m_points2DOFs[1].dof_y], aMBS_GT->m_q[aMBS_GT->m_points2DOFs[1].dof_x] ) );
			...
			for (size_t i=0;i<pf.m_particles.size();i++)
			{
				...
				orientation_averager.m_particles[i].d->phi( atan2( part->num_model.m_q[part->num_model.m_points2DOFs[1].dof_y], part->num_model.m_q[part->num_model.m_points2DOFs[1].dof_x] ) );
				orientation_averager.m_particles[i].log_w = pf.m_particles[i].log_w;
			}

			// Estimate a full (x,y,phi) pose just for easy reusing of MRPT implementation:
			...
			if (pf_out_info.resampling_done)
			{
				...
				for (size_t k=0;k<sensor_descriptions.size();k++)
					sensor_descriptions[k]->sensor_noise_std = PF_SENSOR_NOISE_STD * FACTOR;
			}
			...
			// ==============================================
			{
                ...
                // Update 3D scene:
                win3D.addTextMessage(
                    10,10, mrpt::format("Time: %.03fs | ESS=%.02f | %u samples", t_old_simul,pf_out_info.ESS, static_cast<unsigned int>(pf.particlesCount())),
                    TColorf(1,1,1), "mono",15,  mrpt::opengl::NICE, 0 /* txt ID */, 1.5 /*spacing*/, 0.1 /*kerning*/, true /*draw shadow */ );
...
#if COMPUTE_RMSE
    {
        ...
        {
            sqerr += square( mrpt::math::wrapTo2Pi(GT_ang0[i]) - mrpt::math::wrapTo2Pi(EST_ang0_mean[i]) );
            n++;
        }
...
void pf_initialize_uniform_distribution(CMultiBodyParticleFilter &pf,CModelDefinition &model)
{
	...
	{
		CMultiBodyParticleFilter::particle_t * part = pf.m_particles[i].d;
		const std::vector<TPoint2DOF> & p2dofs = part->num_model.getPoints2DOFs();
		...
		do
		{
			...
			const double ang = mrpt::random::randomGenerator.drawUniform(-M_PI,M_PI);
			const size_t nTotalDOFs = part->num_model.m_q.size();
			for (size_t k=0;k<nTotalDOFs;k++)
				part->num_model.m_q[k] = mrpt::random::randomGenerator.drawUniform(-20,20);

			const size_t PT_IDX = 1; // This point is the one we force to be at a predefined position:
