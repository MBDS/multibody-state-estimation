...
void buildParameterizedMBS( const size_t nx,const size_t ny, CModelDefinition &model, const double NOISE_LEN = 0 )
{
	ASSERT_(nx>=1 && ny>=1)
	...
	{
		const double x = i*Lx; // + (irregular_mesh ? randomGenerator.drawUniform(-NOISE_LEN,NOISE_LEN) : 0 );
		model.setPointCoords(i,TPoint2D(x ,0), true /*is fixed*/ );
	}

	// points definition
	for (size_t row=1;row<=ny;row++) // from row 1 (0 belongs to fixed points) to ny
	{
		for (size_t j=0;j<=nx;j++) // from column 0 to nx
		{
			const double x = j*Lx + randomGenerator.drawUniform(-NOISE_LEN,NOISE_LEN);
			const double y = row*Ly + randomGenerator.drawUniform(-NOISE_LEN,NOISE_LEN);
	...
	for (size_t row=1;row<=ny;row++) // from row 1 (0 belongs to fixed points) to ny
	{
		for (size_t j=0;j<nx;j++) // from column 0 to nx-1
		{
			...
			b.length() = (model.getPointInfo(b.points[0]).coords - model.getPointInfo(b.points[1]).coords).norm();
			b.mass()		= 1*b.length();
...
void buildSliderCrankMBS(CModelDefinition &model )
{
	...
	model.addConstraint( CConstraintFixedSlider(
		2 /*pt index*/,
		TPoint2D(-3,-2), TPoint2D(8,2) /* The line on which to fix the point */
		) );
...
void buildTwoSliderBlocks(CModelDefinition &model )
{
	...
	model.addConstraint( CConstraintFixedSlider(
		0 /*pt index*/,
		TPoint2D(0,0), TPoint2D(0,1) /* The line on which to fix the point */
		) );
	model.addConstraint( CConstraintFixedSlider(
		1 /*pt index*/,
		TPoint2D(0,0), TPoint2D(1,0) /* The line on which to fix the point */
		) );
...
int main(int argc, char **argv)
{
	try
	{
		...
		//buildParameterizedMBS(Nx, Ny, model, 0.6 /*random imperfections in the mesh*/);
...

		{
			...
			//scene->getViewport()->setCustomBackgroundColor( mrpt::utils::TColorf(0.1f,0.1f,0.1f));

			gl_MBS->setPose( mrpt::poses::CPose3D(0,0,0, DEG2RAD(180),DEG2RAD(0),DEG2RAD(90) ) );
		...
		while (win3D.isOpen())
		{
...
#if SIMUL_REALTIME
			if (t_new>t_old+dynSimul.params.time_step)  // Just in case the computer is *really fast*...
			{
				double t_new_simul = t_old_simul + REALTIME_FACTOR * (t_new-t_old);
			...
			// Eval energy:
			if (++ENERGY_DECIMATE_CNT>ENERGY_DECIMATE && E_tot.size() < ENERGY_MAX_LOG)
			{
			...
			if (tictac_gui_refresh.Tac() >= GUI_DESIRED_PERIOD)
			{
				...
				win3D.addTextMessage(
					10,10, mrpt::format("Time: %.03fs (x%.02f) |Phi|=%e", t_old,REALTIME_FACTOR,aMBS->m_Phi.norm()),
					TColorf(1,1,1), "mono",15,  mrpt::opengl::NICE, 0 /* txt ID */, 1.5 /*spacing*/, 0.1 /*kerning*/, true /*draw shadow */ );

				const double simul_t = sparsembs::timelog.getMeanTime("mbs.run_complete_timestep");
				const double simul_Hz = simul_t>0 ? 1.0/simul_t  : 0;
				win3D.addTextMessage(
					10,30, mrpt::format("Simul: %.02fHz [%.02fms]", simul_Hz,simul_t*1e3 ),
					TColorf(1,1,1), "mono",15,  mrpt::opengl::NICE, 1 /* txt ID */, 1.5 /*spacing*/, 0.1 /*kerning*/, true /*draw shadow */ );
				...
				if (win3D.keyHit())
				{
					...
					{
#if SIMUL_REALTIME
					case '+':  REALTIME_FACTOR*=1.1; break;
					case '-':  REALTIME_FACTOR/=1.1; break;
#endif
					default: break;
 					};
		...
		if (SHOW_ENERGY_BALANCE)
		{
			...
			mrpt::gui::CDisplayWindowPlots winE2("Total energy variation",600,300);
