...
TSimulationState::TSimulationState(const AssembledRigidModel* arm_)
	: t(0), arm(arm_)
{
}
...
// ---------------------------------------------------------------------------------------------
CDynamicSimulatorBase::CDynamicSimulatorBase(
	std::shared_ptr<AssembledRigidModel> arm_ptr)
...
{
	ASSERT_(pnt_index < arm_->mechanism_.getPointCount());
	...
	const Point2* mbs_point = &arm_->mechanism_.getPointInfo(pnt_index);
