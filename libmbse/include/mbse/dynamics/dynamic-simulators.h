...
class CDynamicSimulatorBase
{
   public:
	...
	CDynamicSimulatorBase(std::shared_ptr<AssembledRigidModel> arm_ptr);
	...
	AssembledRigidModel* get_model_non_const() const { return arm_ptr_.get(); }
   ...
   protected:
	...
	const std::shared_ptr<AssembledRigidModel> arm_ptr_;
	AssembledRigidModel* const arm_;  //!< The prepared MBS model.
...
class CDynamicSimulatorIndepBase : public CDynamicSimulatorBase
{
   public:
	...
	CDynamicSimulatorIndepBase(std::shared_ptr<AssembledRigidModel> arm_ptr);
	 ...
	 *  You MUST call prepare() before this method.
	 */
	void solve_ddotz(double t, Eigen::VectorXd& ddot_z);
	...
	/** Whether to use automatic determination of DOF z variables. If set to
	 * false, the vector indep_idxs_ must be set manually before solving for
	 * accelerations. */
	bool can_choose_indep_coords_ = true;
...
class CDynamicSimulator_Indep_dense : public CDynamicSimulatorIndepBase
{
   public:
	...
	/** Manual selection of independent coordinates. Calling this method also
	 * sets can_choose_indep_coords_=false */
	void independent_coordinate_indices(
		const std::vector<size_t>& idxs) override
	{
		indep_idxs_ = idxs;
		can_choose_indep_coords_ = false;
	}
...
/** Dense solver with the index-3 Augmented Lagrange formulation (ALF) with
 * projection of velocities and acelerations */
